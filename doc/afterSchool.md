...
정말 거의 매일 회의, 발표, 과제 이런 부담들 때문에 화요일 5시까지 고민하다가 가게 되는 것 같습니다. 오늘은 뒷풀이도 못가고 PPT 만들러 일찍 왔는데, 막상 컴퓨터 켜고 후기 쓰고 있네요. 헤헤헤
고민은 해도, 가서 강의도 -무려 재미있는!- 듣고, 여러 사람들도 만나면 역시 '다음주에 또 와야지~' 이런 결심과 함께 전철을 타지만, 막상 화요일 5시가 되면 또 고민하고 있는 무한루틴의 삶? ㅎㅎㅎ
제가 처음 듣는 주제에 그냥 듣기만 해서, 아스라히 기억나는 대로 정확하지 않을 가능성이 농후한 후기 올리는 점 양해 바랍니다. ^^;;

<br>오늘은 Sungjin Park님의 영상처리에 대한 소개와 Kang-ik Kevin Cho님의 fMRI 처리에 관한 발표가 있었습니다.
저의 EEG 발표 때 PPT를 보고 반성이 되었다고 하셨는데, 제가 매의 눈으로 살펴본 결과 반성이 별로 반영된 것 같지는 않은 PPT였습니다. ㅎㅎ 그렇지만 내용을 아주 쉽게 설명을 잘 해주셨어요!
일단 coronal, saggital, axial 영상을 어떻게 얻는지에 대한 설명부터, 영상의 해상도를 행렬로 처리해서 비트맵으로 계산하는 것. 또, MRI 한 번 찍으면(?) 2의 9승 블라블라 라는 과정을 거쳐ㅋ 60MB를 차지하는 CD 1장에 100장 정도 들어가는 데이터가 나오는데, 최신 좋은 기계는 만장~10만장 정도의 데이터가 나오기 때문에, 이 역시 빅데이터 분야가 되고 있다고 말씀하셨습니다. 그리고 MRI를 촬영할 때, 사람이 움직일 수 밖에 없기 때문에 (즉, 데이터 값이 일치하는 위치에서 나오기 어렵기 때문에) T1, T2를 이용해서 위치를 파악하는 방법을 설명해 주셨습니다. 이 부분에 대한 이해가 쉽지 않았는데, 중간에 제가 찾아 본 논문을 첨부합니다. (저는 T1, T2가 뭔지 몰라서 찾아 봤는데, 도움이 되는 설명이 많더라고요.)
이후 어김없는 자기 소개 시간이 있었습니다. ㅎㅎㅎ

<br>그리고 무려 전공자의 '여유'를 온화한 코믹 발표 자료로 승화하신 조강익님의 발표가 있었습니다. (저도 그건 어떤 툴로 만든 건지 궁금합니다. PPT와 PREZI정도만 알고 있었는데, 새로운 툴인가요? @_@!!)
fMRI는 유명세에 비해 해상도가 낮다는 약점이 있더군요. 그래서 해상도 깡패인 sMRI(structural MRI)에 대한 설명을 해주시고 (T1/T2, DTI등) 이 두 이미지를(from fMRI & sMRI) mapping함으로써 연구자가 관찰하고 싶은 부위를 찾아, crop + ctrl V한다는 설명이었습니다.
사실 내용만으로도 매우 흥미롭고 '저걸 남이 한 걸 보고 있으니 재미있지, 내가 하려고 하면 엄청 삽질이겠다' 싶은 안도감을 주는 화려한 결과물을 보는 것도 재미있었는데, 중간 중간 삽입되어 있는 이미지 떡밥들이 강익님의 센스와 합쳐져서 정말 'SHOW'를 보는 느낌이었습니다.
뇌도 동글동글 귀엽게 그리시더군요. ㅎㅎㅎ 그리고 데이터 처리를 병렬분산으로 하고 싶은데, 서버와 속도로 고민이라는 말을 던지자마자 여러 멤버분들이 '공유기' '공유기지' '기가바이트 랜선' 하는 해답 들을 마구 쏟아 내시면서 윈-윈하는 현장을 목격할 수 있었습니다.
두 분 다 짧지 않은 발표였는데, 흥미롭게 잘 설명해 주셔서 무척 도움이 되는 자리였습니다. 제가 그 비싼 fMRI로 실험을 할 수 있게 될는지는 모르겠지만, 어쩐지 발표만으로도 가까워진 느낌이 듭니다.
강익님 훈련소 잘 다녀오시고, 상남자되어서 ㅋㅋ 돌아오시기 바랍니다~!!

오늘 많은 분들 오셨는데, 다들 반가웠습니다!!
권나연_150428
